...
import { denyFriendReq, sendFriendReq } from "@/lib/friendActions";
    ...
    <div className="flex w-full gap-2 items-center">
      <div className="flex-1 grid grid-cols-[48px_1fr] gap-4">
        ...
        <div className="flex flex-col items-start justify-between">
          <span className="font-semibold">{request.sender.name} - @{request.sender.username}</span>
...
export const OutgoingFriendRequest = ({ request }: { request: friendRequest }) => {
  return (
    <div className="flex w-full gap-2 items-center">
      <div className="flex-1 grid grid-cols-[48px_1fr] gap-4">
        <Avatar>
          <AvatarImage src={request.receiver.image} referrerPolicy="no-referrer" className="grid-rows-2 size-12"/>
          <AvatarFallback className="text-lg grid-rows-2 size-12">{request.receiver.name.substring(0,1).toUpperCase()}</AvatarFallback>
        </Avatar>

        <div className="flex flex-col items-start justify-between">
          <span className="font-semibold">{request.receiver.name} - @{request.receiver.username}</span>
          <span className="text-sm text-muted-foreground">{new Date(request.createdAt).toLocaleDateString()}</span>
        </div>
      </div>
