...
import { updateUser, uploadImage } from "@/lib/dbActions";
...
const Account = () => {
  ...
  const changeProfile = async( e: FormEvent<HTMLFormElement> ) => {
    ...
    const changes: changesType = {
      ...
      username: (formData.get('username') as string).trim().toLocaleLowerCase().replaceAll(" ", "_"),
      image: '',
      socials: {
    ...
    changes.socials = JSON.stringify(changes.socials)

    // Send data to Database
    const status = await updateUser({userID: session?.user.id as string, data: changes, url: path});
  ...
  return (
    <section className="container mt-8 flex-1 flex flex-col">
