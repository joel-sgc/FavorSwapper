...
interface CameraProps {
  ...
  closeOnFirstPicture?: boolean
}

const Camera: FC<CameraProps> = ({ onClosed, onCapturedImages, closeOnFirstPicture = false }) => {
  ...
  const handleCapture = (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (camera.current) {
      ...
      if (imageData) {
        ...
        if (closeOnFirstPicture) handleOnCapturedImages([imageData]);
      }
    }
  };
      ...
      <div className="relative w-full">
        <div className="absolute z-10 w-full md:h-[calc(93vh_-_theme(spacing.12))] md:w-[20%] ">
          <div className="relative left-4 top-4">
            <Button
              className="rounded-full p-4 opacity-60 hover:opacity-100  "
              size={"icon"}
          ...
          {(closeOnFirstPicture && images.length > 0) && (
            <div className="absolute right-4 top-4 z-10 md:bottom-0 md:left-2 md:right-0 md:top-auto ">
              <Button
                className="rounded-full p-4 opacity-60 hover:opacity-100  "
                size={"icon"}
                variant={"secondary"}
        ...
        <div className="absolute bottom-2 left-1/2 transform -translate-x-1/2 z-20 md:bottom-auto md:left-auto md:right-14 md:top-[50%] ">
          <Button
            className={cn(
              "group h-12 w-12 rounded-full p-8 opacity-60 hover:opacity-100  ",
            )}
            size={"icon"}
            variant={"default"}
            onClick={(e) => {
        ...
        >
          {images.length > 0 && (
            <div className="absolute bottom-4 left-4 md:bottom-auto md:left-auto md:right-14 md:top-0">
              <Gallery />
            </div>
          )}

          {numberOfCameras > 0 && (
            <div className="absolute bottom-4 right-4 z-10 md:bottom-0  md:right-14  md:top-auto">
              <SwitchCamera />
            </div>
          )}
        </div>
          ...
          <div className="grid grid-cols-2 gap-2  ">
            {images.map((image, index) => (
              <div key={index} className="relative ">
                ...
                <Button
                  className="absolute right-2 top-2 h-6 w-6 rounded-full  p-2 opacity-60 hover:opacity-100  "
                  size={"icon"}
                  variant={"destructive"}
                  onClick={() => {
                    removeImage(index);
