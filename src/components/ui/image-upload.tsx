...
const ImageUpload = React.forwardRef<HTMLInputElement, InputProps>(
  ({ file, onChange, className, type, ...props }, ref) => {
    return (
      ...
      >
        <CloudUpload size={64}/>
        <div className="font-semibold text-foreground/90 text-center">
          {(file && file.size > 0) ? (`${file.name} - ${Math.round((file.size / 1000 / 1000) * 100) / 100} MB`) : "Drag 'n' drop some files here, or click to select files"}
        </div>
